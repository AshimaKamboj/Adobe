...
from utils.pdf_utils import summarize_text


def extract_text_with_ocr(page, lang="hin+eng+jpn"):
    """Perform OCR on the page image using Tesseract."""
    pix = page.get_pixmap()
    img_bytes = pix.tobytes("png")
...
def extract_outline_and_sections(pdf_path, file_name):
    ...
    for page_num, page in enumerate(doc, start=1):
        ...
        for line in lines:
            ...
            # Heading detection (any language)
            if len(line.split()) <= 10 and line[0].isupper():
                level = "H1" if len(line.split()) <= 3 else "H2"
